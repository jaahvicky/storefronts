/* MODIFIED BY METHYS TO ADD METHODS AND FEATURES
 * 
 * - added displaying of options on page load
 * - added change events
 * - added triggering of change only if selected values actually changed.
 * 
 * ========================================================================
...
(function ($) {
	...
	var selectedValues = []; //which values were selected when dropdown opened.
	...
	proto.toggle = function (event) {
		...
		if (!isActive) {
			...
			//store list of selected options
			selectedValues = getSelectedItems(opened);
		}
	...
	function getSelectedItems(parent) {
		var selected = [];
		var inputs = parent.find('input[type=checkbox]:checked, input[type=radio]:checked');
	...
	function closeOpened(event, menuTree) {
		if (opened) {
			...
			//check if selected options changed.
			var selected = getSelectedItems(parent);
			if (!compareSelectedValues(selected, selectedValues)) {
	...
	/**
	 * Compares values to see if they are the same.
	 ...
	 * @returns {boolean}
	 */
	function compareSelectedValues(values1, values2) {
