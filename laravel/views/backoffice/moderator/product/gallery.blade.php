    ...
    <div class='box-body'>
        ...
        <?php for ($i=0; $i<5; $i++): ?> 
            ...
            <div class="form-group">
             ...
             @php $img_old = "images-".$i; 
                $srcs = asset('storage/'.old($img_old));
             @endphp
            <div class="form-group fc-fileupload {{(isset($images) && isset($images[$i]) && !empty($images[$i])) ? 'has-image' : old($img_old)}} clearfix">
               
                <input type='hidden' name='images-{!!$i!!}' data-mfu-value='filename' value='{{  (isset($images) && isset($images[$i]) && !empty($images[$i])) ? $images[$i] : old($img_old) }}'/>
                <div>
                    <div class='fc-fileupload-image fc-fileupload-image-{!!$i!!}' 
                        style='background-image: url({{  (isset($images) && isset($images[$i]) && !empty($images[$i])) ? LookupHelper::getImage($images[$i]) : ((!empty(old($img_old)))? $srcs: asset('images/samples/image-holder.jpg')) }});'
